...
--perspektywa dla wszystkich klientów indywiualych
...
FROM klienci_poznan WHERE czy_biznesowy = 1;

--cała sprzedaż
CREATE OR REPLACE VIEW cala_sprzedaz 
AS SELECT * FROM sprzedaz 
UNION
SELECT * FROM sprzedaz_poznan;
...
INNER JOIN mv_ubezpieczenia ubezp 
...
--perspektywa dla klientów z Poznania
CREATE OR REPLACE VIEW klienciPoznan
AS SELECT * FROM klienci_poznan ORDER BY id_klient ASC;